...
- 01-supabase-service-role-server-only.ts: SERVICE_ROLE_KEY behind "server-only" import
- 02-public-config-vars.ts: NEXT_PUBLIC_ values that are legitimately public (anon key, site URL)
- 03-firebase-admin-server-lib.ts: firebase-admin in src/lib/server with env-loaded creds
- 04-stripe-in-getserverside.tsx: STRIPE_SECRET_KEY only in getServerSideProps
- 05-stripe-test-keys-fixtures.ts: pk_test_/sk_test_ keys in a *.test.ts fixture
- 06-aws-keys-from-env.js: AWS creds read from process.env at boot, fail-fast if missing
- 07-anthropic-server-only.ts: Anthropic client initialized in a "server-only" module
- 08-secrets-decrypted-from-kms.ts: secrets read encrypted at rest, decrypted via KMS at boot
- 09-slack-webhook-from-env.py: SLACK_OPS_WEBHOOK loaded from os.environ
- 10-jwt-secret-from-env.go: JWT signing key initialized from env in init()
