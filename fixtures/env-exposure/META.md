...
- 01-admin-env-gated.ts: /admin/env requires admin AND production check, returns subset
- 02-public-config.ts: /api/config returns hand-picked typed PublicConfig
- 03-fastify-redacted-logs.ts: env logged but routed through pino redact list
- 04-dev-env-keys-only.ts: dev route returns env KEYS only, 404 in prod
- 05-healthz-specific-fields.js: /healthz returns only region/version/uptime
- 06-logger-only-env.js: process.env passed only to redacting logger, not response
- 07-redacted-diagnostics.js: regex-redacts SECRET/KEY/TOKEN before responding
- 08-flask-env-keys-only.py: Flask returns sorted env key names without values
- 09-fastapi-runtime-specific.py: FastAPI returns specific allowlisted env values
- 10-go-public-env.go: Go handler returns predefined safeKeys subset only
