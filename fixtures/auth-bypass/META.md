...
- 23-blanket-use-inverted-claims-guard.ts: Express router with `policiesRouter.use(requireAuth)` and eight bare sibling routes; the destructive claims route's ownership conditional is inverted, so it denies the holding broker and permits every other authenticated caller. Pairs with negative/24, which differs by one negation operator. Prediction: fixtures/_pending/auth-bypass-blanket-use/PREDICTION.md
- 24-subscribers-app-no-identity-anywhere.js: Express module with NO auth middleware on any route and no session machinery; handlers pass a caller-supplied scalar to services that never receive the request, so no authorization check is reachable. Lands in case 2's all-siblings-unguarded fallback and tests its "unambiguously destructive" exception. Pairs with negative/25. Prediction: fixtures/_pending/auth-bypass-crossfile-auth/PREDICTION.md
...
- 24-blanket-use-all-covered.ts: Express router with `policiesRouter.use(requireAuth)` and eight bare sibling routes, every one correctly covered; the claims route's ownership conditional is the right way round. Symmetric anchor to positive/23 — the code differs by one negation operator, so a flag here is attributable to the blanket shape rather than to any defect (Category B — context)
- 25-members-app-handler-internal-auth.js: Express members module derived from TryGhost/Ghost@ead70448; 3 of 9 routes carry an auth middleware argument and 6 do not, with the 6 authenticated inside their handlers cross-file (verified against Ghost's middleware.js and member-controller.js). Case 2's "strong tell" configuration pointing at correct code. Symmetric anchor to positive/24 (Category B — context)
