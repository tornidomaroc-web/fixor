...
**Status: REMEDIATION STARTED.** Every row's state is the one its own Status column
gives. This banner deliberately does NOT restate which findings are open: a summary
of a list, sitting above that list, goes stale the moment the list moves and is read
as current anyway. It previously read "all other findings below remain open or
diagnostic", which PR #141 falsified by resolving F-003 without touching this line.
This is the running record of every defect surfaced during the production-readiness
audit. Each phase appends here; remaining fixes land through the protected-main PR
gate as the audit closes out.
...
|----|-------|----------|-----------|-----------|--------|-------------|
| F-001 | Cross-file parent-layout auth guard not wired on the shipped webhook/API path | HIGH | B | `workflows/auditor-workflow.ts:371` (detect called with `{diff}` only, no `sidecarsByPath`); cf. `cli/scan.ts:414-422` where Engine A resolves it | **RESOLVED - merged in PR #75 (squash 5e26b00e16d42822b882fe397ff30191fe3f34a6)** | **RESOLVED (PR #75, squash 5e26b00):** Engine B (`runAuditorWorkflow`) now resolves ancestor `_layout` guards over the PR head and forwards `sidecarsByPath` to `detect()` (`workflows/auditor-workflow.ts`); regression guarded by committed fixture `fixtures/f001-layout-guard/` plus `src/test/test-route-guard-webhook-wiring.ts` (deterministic CI) and `src/test/test-f001-webhook-parity.ts` (opt-in live parity). Original confirmation (kept for record): Engine A passes `sidecarsByPath[ROUTE_GUARD]` from `resolveRemixRouteGuard`, so a Remix/RR-v7 route gated by an ancestor `_layout.tsx` loader is correctly cleared. Engine B never builds that sidecar, so the same route reaches the LLM with no parent-guard block - the Phase-G false-positive defense is absent on the real product. Measured against a constructed neutral layout-gated fixture (`dashboard+/billing.tsx`, PROVEN-blocking `_layout.tsx` loader, no auth-suggesting naming or comments), **6 runs per engine, byte-identical input (Phase 3D): Engine A cleared 6/6** (isVulnerable=false, LOW, authPresent=yes via sidecar); **Engine B false-positived at HIGH 6/6** (`auth_bypass_risk`, "GET /dashboard/billing", authPresent=no). The divergence is **deterministic**, not a fluke - this is the headline readiness finding on solid repeated evidence. Blast radius: the FP fires specifically on layout-gated routes that do NOT advertise auth in folder name or comments (see F-013). |
| F-002 | Non-LLM detector throw is logged but not counted as degraded coverage | MEDIUM | B (the A half is resolved) | `cli/scan.ts:458-463` (`logger.warn`, continue); cf. `workflows/auditor-workflow.ts:383-395` (Sentry-captured but also continues) | **OPEN, NARROWED to Engine B (Engine A half resolved in PR #141, squash 38d5c7ca)** | **NARROWED (PR #141, squash 38d5c7ca), NOT closed.** On Engine A a detector that throws is now recorded as a named casualty (`FileScanResult.detectorFailures`, carrying the detector id), rendered by name under "Coverage gaps", and counted toward the exit code by `countCoverageDegradations`. **The defect is untouched on Engine B**, `runAuditorWorkflow`, which is the CUSTOMER-FACING path, so the remaining severity sits where the blast radius is largest and the row stays OPEN. Read the residue precisely: Engine B still captures a detector throw to Sentry and continues without degrading workflow status. Original diagnosis (kept for record): If a detector throws for a non-LLM reason (parser bug, unexpected input), the CLI swallows it as a warning and that detector contributes zero findings for that file. The `llm-coverage` integrity gate only catches *LLM-call* failures, so a non-LLM throw does not flip exit code 2 / workflow error - a partially-blind run can still read as clean for that failure class. Engine B captures to Sentry (better) but still does not degrade status. |
| F-003 | `readFileSync` failure yields zero findings with no coverage signal | MEDIUM | A | `cli/scan.ts:465-467` | **RESOLVED - merged in PR #141 (squash 38d5c7cae71a97c2f3924d7a946d4f8e1734000e)** | **RESOLVED (PR #141, squash 38d5c7ca):** a file whose analysis aborts is now recorded as a named casualty on the per-file result (`FileScanResult.notAnalyzed` in `cli/report-builder.ts`, carrying the stage it died at so a permissions problem is distinguishable from a bug), rendered BY NAME in the report's "Coverage gaps" section, excluded from the analyzed count (the report now separates files enumerated from files fully analyzed), and composed into the exit code by `countCoverageDegradations`, which feeds the same number to the banner, the summary block and `coverageExitCode` so the report and the exit code cannot disagree. BREAKING: such a run exits 2 where it previously exited 0. Regression guarded by `src/test/test-scan-coverage-integrity.ts` (deterministic, keyless, wired into `test:ci` as `test:scan-coverage`). Scope is Engine A only, which is this row's whole scope. Original diagnosis (kept for record): If a file can't be read at scan time (permissions, race, encoding), it is logged at error level and silently contributes no findings. It is not counted as an LLM coverage failure, so the scan's "clean/degraded" verdict and exit code are unaffected - a file that was never analyzed is indistinguishable from a file analyzed and found clean. |
