...
- **It changes no gating status.** The reach finding is now tracked as **L-012** (OPEN; reach / market-fit; NOT a defect; NON-gating) — a readiness call the owner has since made. L-012 changes no gate: reach says nothing about detection quality on a real vulnerability (that was C's job). C has since run: the langflow C run lifted L-010 to LIFTED PARTIAL (`REMEDIATION-PROGRESS.md`). L-012 neither lifted nor pressured F-004 or L-010; the lift came from the C run, not from reach. Companion item: **L-013** (`REMEDIATION-PROGRESS.md`, Priority 1f) owns framework-idiom coverage (which code idioms the prefilter reaches on a probe over recently-patched ownership fixes) where L-012 owns market applicability (2/43). The two interlock and share no figure: L-012 is the ICP-market reach fact, L-013 is the idiom shape.
