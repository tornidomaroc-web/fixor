...
> **SUPERSEDED IN MECHANISM, NOT IN LESSON — read before acting on either dated passage below. Annotated 2026-07-31. Cause: PR #120. Both passages are left exactly as entered.**
>
> This note covers BOTH dated passages that follow: the `(Note 2026-06-12: ...)` parenthetical inside "Estimate validation", and "Second instance (F-004 stage-3 step 2, 2026-07-22)". It is one block because they are one defect, not two. Both describe a per-call pricing mechanism that PR #120 removed, and reading them as two independent corrections would suggest each needs its own remedy. It sits above them rather than below because a reader who stops at the first passage would otherwise never reach the correction.
>
> WHAT #120 DID. It stopped pricing by multiplying a call count by a per-call rate, and made the harness report MEASURED cost read from the call ledger. It did not give the rate a better value; it deleted the thing that needed one. `stability-harness.ts` now computes a projection only when an entry point supplies `costPerLlmCallUsd`, with no fallback branch, so there is no harness default, no flat constant to fit some call shape worst, and nothing for IDOR to inherit. IDOR's two stability entry points supply no rate, so they emit `no projection (no rate supplied)`.
>
> WHAT THAT CHANGES IN EACH PASSAGE. The 2026-06-12 note ends "The harness default has been corrected", which presupposes a default that still exists and was merely revalued; it was removed instead. The 2026-07-22 passage locates the residual pricing risk in "whichever call shape the flat constant fits worst (here IDOR, which is whole-file and batched and inherits the harness default)"; neither the constant nor the default remains to be fitted or inherited.
>
> WHAT IS UNTOUCHED, because it is independently true. Both numbered lessons of the 2026-07-22 passage stand, as does its instruction to name the residual. So does the 2026-06-12 lesson that independent pre-spend cost math needs the right per-call rate: a projection made BEFORE a run has no ledger to read and still rests on an estimate. The residual is still real, because a canned response carries zero token usage, so a zero-spend run still cannot price. What changed is who names it: the harness now reports its own measured cost at run time instead of letting a constant stand in for a price, which is the 2026-07-22 passage's own instruction implemented in code rather than left to the reader.
