...
- [x] **5F-4** Mintlify docs site: setup guide, supported languages, detector catalogue, API reference, FAQ. (PR #47 — `docs/mintlify/` ships docs.json + 6 MDX pages: introduction (CardGroup overview of all four detectors), quickstart (Steps walkthrough install→first PR→tune→budget), detectors (per-family with detector ids, supported frameworks, before/after fix samples), languages (status table JS/TS shipping, Python/Java/Go on roadmap, Ruby/PHP considering), api-reference (POST /api/v1/scan with token auth via the existing `npm run create-api-token` CLI, full request/response/error shape), faq (Accordion groups for pricing, privacy, accuracy, operations). docs.json wires the CTA to the GitHub App install URL, "Sign in" to app.fixor.dev, GitHub + Status anchors, footer links to privacy/terms/security pages on the landing. `docs/MINTLIFY-SETUP.md` is the operator runbook — Mintlify account, GitHub integration, custom-domain DNS for `docs.fixor.dev`, pre-launch checklist incl. find/replace of `support@fixor.dev` placeholder. Mintlify account creation + monitor configuration + DNS itself is the operator's action.)
- [x] **5F-5** Public README polish: "Install on GitHub" badge linking to marketplace, screenshots, comparison table vs Snyk/Semgrep. (PR #48 — README rewritten end-to-end. Hero gains an "Install on GitHub" shield linking to the App install URL alongside the Claude/MIT badges and a one-line link bar (Landing · Dashboard · Docs · Status · Security). Capability table now reflects the four shipping detectors + dashboard + Paddle billing instead of "XSS / cmdi / pt on roadmap" copy that predated 5B-4. Honest Snyk/Semgrep comparison table positioned as "fewest steps to a useful finding" rather than feature-for-feature competition (different tools for different scales). Tech-stack table refreshed with Drizzle/Neon/Pino/Sentry/Clerk/Paddle/Resend/Better Uptime — every layer Phase 5 introduced. Project-structure block updated to match current src/ layout (db/, lib/, apps/dashboard/, etc.). Security section points at the trust-center page. Screenshots block ships as commented-out `<img>` tags with a pointer at `docs/MARKETPLACE-LISTING.md`'s asset checklist — operator drops PNGs at `docs/screenshots/<name>.png` and uncomments.)
- [x] **5F-6** Launch posts (write but don't post yet): X thread, HN Show, Indie Hackers, ProductHunt teaser. (PR #49 — `docs/LAUNCH-POSTS.md` ships four full drafts, each in its platform's voice. Show HN: dry+technical with the cost-cap math + "why Paddle not Stripe" hook for HN's geo-aware crowd, MIT-license reminder, expects critique. X thread: 7 tweets, hook-first ("kept committing SQL injection in my own side projects"), screenshot cues per tweet, screenshot placeholders sourced from the marketplace asset checklist. Indie Hackers: journey-format build log with the actual phase sequence, three lessons learned (budget-caps-as-feature, Paddle-over-Stripe, dashboard-not-optional), real revenue goal, ~$1k MRR target. Product Hunt: tagline + short + long descriptions + a 60-sec-after-launch first comment with three feedback prompts + topics + gallery image order + the "00:01 PT, not morning" timing trick. Plus a posting-day calendar (5F-7 reference) and after-launch follow-ups blocked at day 2 / 5 / 7. Pre-launch find/replace block at the top of the doc lists every placeholder (`<install-url>`, `<demo-gif>`, etc.) so paste-and-go is one search-and-replace per token. **Drafts only — none scheduled, none posted.**)
- [x] **5F-7** **Launch day**: schedule for a Tuesday. Post HN at 8am ET. X thread same time. PH 12am PT. Reddit r/programming at 9am ET. Be online for 8 hours to answer questions. (PR #50 — `docs/LAUNCH-DAY.md` is the operator playbook: T-1 readiness checklist (code/infrastructure/public-surfaces/comms/draft-swap), minute-by-minute Tuesday timeline (PH 00:01 PT → IH 06:00 ET → HN + X 08:00 ET → Reddit 09:00 ET → 8 hours of live presence), 15 canned responses to predictable Q&A across product/privacy/billing/operations/OSS so launch-hour replies are 60s instead of stewing for 20, what-good-looks-like rubric, hotfix protocol, after-launch follow-up cadence (day +2 retro, +5 cohort post, +7 v0.1.0 tag). Combined with the 5F-6 drafts, launch day is paste-and-go. Posting the actual launch is the operator's call — Phase 5F's deliverable for this task is the playbook itself.)
