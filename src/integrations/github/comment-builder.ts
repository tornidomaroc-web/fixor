...
export function buildPullRequestCommentMarkdown(
  ...
  const lines: string[] = [
    "## Fixor · Security vulnerability report",
    "",
  ...
  lines.push(
    ...
    `| **Semgrep findings (total)** | ${workflow.totalFindings} |`,
    `| **Vulnerabilities found** | ${workflow.sqlInjectionFindings} |`,
    `| **Skipped (other rules)** | ${workflow.skippedFindings} |`,
  ...
  if (list.length === 0) {
    ...
    lines.push(
      ...
      `<sub>Generated by Fixor · finished at \`${workflow.timing.finishedAt || "—"}\`</sub>`
    );
    return lines.join("\n");
  }
  ...
  detailedList.forEach((fix, i) => {
    ...
    if (exploit) {
      ...
      const payloadCell = cell(exploit.payload.replace(/`/g, "'"));
      ...
      lines.push(
        ...
        "",
        fencedCodeBlock(truncate(exploit.proofOfConcept, 4000)),
        "",
        "</details>",
        ""
      );
