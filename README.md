...
| 🏗️ Framework scope — route-shape detection covers router-style (`router.METHOD(path, ...)` — Express family) AND Next.js App Router method-named exports (`export const POST = ...` / `export async function GET(...)`). App Router calibration measured against inbox-zero: 182 of 228 routes scanned (~80%, PARTIAL scan, halted for API budget — not a full-corpus sweep): 0 auth-bypass / 0 admin-check findings on the scanned 182, plus 2 webhook findings of documented-limitation classes. Phase F (2026-05-23) lifted the cross-file verifier helper class (c) and the non-HMAC shared-secret challenge class (d) from HIGH to MEDIUM/review-queue **when the verification signal is visible to the detector** (verification-suggesting import path or helper call within the route-declaration context window); measured outlook/webhook drop from HIGH to MEDIUM, plus a residual window-truncation case (apple/webhook) where the verifier call sits beyond the windowed payload — that residual remains HIGH and is the named Phase F structural follow-up. Remix `loader` / `action` exports are NOT yet covered (Phase E follow-up; statically measured 0/411 prefilter matches on Trigger.dev). See [`docs/detector-capabilities.md`](docs/detector-capabilities.md) for the four documented webhook limitations and the unfamiliar-wrapper MEDIUM-confidence review-queue UX. | 🟢 App Router · 🟡 Remix planned |
| 🐍 Python · 🐹 Go — IDOR, env-exposure, secrets-exposure, and webhook-unverified detect cross-language today; auth-bypass and admin-check catch their sentinel/hardcoded shapes cross-language but their route-shape sub-claims (missing-middleware Express + missing-HOC-wrapper App Router) are JS/TS-only (see framework-scoping row above and [`docs/detector-capabilities.md`](docs/detector-capabilities.md)) | 🟡 Partial |
