...
| 🐍 Python · 🐹 Go — IDOR, env-exposure, secrets-exposure, and webhook-unverified detect cross-language today; auth-bypass and admin-check catch their sentinel/hardcoded shapes cross-language. **auth-bypass route-shape now covers FastAPI / Flask-2.0 `@app.METHOD` / `@router.METHOD` decorators (Python slice 1, 2026-05-28)** — in-file `Depends`/`Security` name-convention auth recognition, lang-gated to `.py`; validated on the real `full-stack-fastapi-template` (guarded routes cleared, authless route flagged). Cross-file Depends + conditional/router-level mount and admin-check FastAPI are deferred slices (see [`docs/detector-capabilities.md`](docs/detector-capabilities.md)). Go route-shape and Express/App-Router/Remix remain as before | 🟡 Partial (Python auth-bypass route-shape improving) |
